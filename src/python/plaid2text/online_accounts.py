...
class PlaidAccess():
    ...
    def _save_account_section(self, nickname):
        ...
        a = prompt(
            'What currency does this account use for posting transactions [{}]: '.format(
                cm.CONFIG_DEFAULTS.currency
            )
        )
        if a:
            section['currency'] = a
    ...
    def _present_file_options(self, nickname, file_type, default_file):
        a = prompt(
            'Create a separate {} file configuration setting for this account [Y/n]: '.format(
                file_type
            ),
        ...
        if not bool(a) or a.startswith('y'):
            ...
            while cont:
                path = prompt(
                    'Enter the path for the {} file used for this account [{}]: '.format(
                        file_type,
                        default_file
                    ),
                ...
                if path:
                    ...
                    if not file_exists:
                        create = prompt(
                            '{} does not exist. Do you want to create it? [Y/n]: '.format(path),
                            validator=YesNoValidator()
                        ).lower()
                    ...
                    create = prompt(
                        '{} does not exist. Do you want to create it? [Y/n]: '.format(
                            default_file
                        ),
                        validator=YesNoValidator()
                    ).lower()
    ...
    def _present_question(self, question):
        ...
        answer = prompt(
            ...
            validator=NullValidator(message='You must enter your answer',
                                    allow_quit=True))
        if answer.lower() == 'q':
            return False  # We have abandoned ship
    ...
    def _present_list(self, data):
        ...
        answer = prompt(
            ''.join(message),
            validator=NumberValidator(
                message='Enter the NUMBER where you want to send verification code',
                allow_quit=True,
