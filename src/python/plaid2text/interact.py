        ...
        for w in self.words:
            if word_matches(w):
                ...
                if text_len == next_colon: #next char is colon
                    ...
                    if next_colon < 0 : next_colon = len(w)
                    ...
                    if last_word == display_word.lower(): continue
                    ret = (w,display_word)
                else:
...
class NumberValidator(NullValidator):
    ...
    def validate(self, document):
        ...
        text = document.text
        if self.allow_quit and text.lower() == 'q': return
        if not text.isdigit():
            i = 0
        ...
        if not bool(self.max_number): return
